...
<body>
  ...
  <h1>Phòng trọ xem nhiều nhất</h1>
  <div align="center">
  ...
  <?php 
  $limit = 4;
  $sql = "select * from motel where title like '%$search%'";
  $result = mysqli_query($connect,$sql);
  $pages_max = ceil(mysqli_num_rows($result) / $limit);
  if (!empty($_GET['pages'])) $pages = $_GET['pages'];
  else $pages = 1;
  $sql = "select * from motel where title like '%$search%' limit ".($pages-1)*$limit.",".$limit;
  $result = mysqli_query($connect,$sql);
  ...
  <div style="display: table; margin: 0 auto;">
    <?php 
    for ($i = 1; $i <= $pages_max; $i++) echo "<a class='page'  href='?search=$search&ok=search&pages=$i'>$i</a>"
    ?>
  </div>
  <?php
    }
